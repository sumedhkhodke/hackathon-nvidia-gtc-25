...
def main():
    ...
    if prompt := st.chat_input("Ask me anything about your lifelog data..."):
        ...
        with st.chat_message("assistant"):
            # Create placeholder for streaming intermediate steps
            ...
            try:
                for event in workflow.stream(prompt):
                    if event["type"] == "intermediate":
                        # Update intermediate thinking steps in real-time
                        all_reasoning_steps = event.get("reasoning_steps", [])
                        all_safety_checks = event.get("safety_checks", [])
                        react_cycles = event.get("iteration_count", 0)
                        
                        # Display intermediate steps with animation
                        with thinking_placeholder.container():
                            st.markdown("### 🧠 Agent Thinking Process")
                            with st.expander("🔍 View Real-Time Reasoning (ReAct Pattern)", expanded=False):
                                for step in all_reasoning_steps:
                                    st.markdown(f"""
                                    <div class="reasoning-step streaming-step">
                                        <span class="step-title">{step['step']}</span><br/>
                                        {step['description']}
                                    </div>
                                    """, unsafe_allow_html=True)
                        ...
                        if event["success"]:
                            ...
                            # Clear thinking placeholder and show final response
                            ...
                            # Show collapsible reasoning steps
                            with st.expander("🔍 View Complete Agent Reasoning Process (ReAct Pattern)"):
                                for step in all_reasoning_steps:
                                    st.markdown(f"""
                                    <div class="reasoning-step">
                                        <span class="step-title">{step['step']}</span><br/>
                            ...
                            if all_safety_checks:
                                with st.expander("🛡️ Safety Guardrails"):
                                    for check in all_safety_checks:
                                        ...
                                        st.markdown(f"""
                                        <div class="reasoning-step safety-check">
                                            <span class="step-title">{icon} {check_type.title()} Check</span><br/>
                                            Status: {'Safe' if is_safe else 'Flagged'}<br/>
                                            Category: {check.get('category', 'N/A')}
                                        </div>
                            ...
                            # Show stats
                            st.info(f"📈 Analyzed {retrieved_entries} relevant entries | 🔄 ReAct Cycles: {react_cycles}")
                        else:
                            error_msg = f"❌ Sorry, I encountered an error: {event.get('error', 'Unknown error')}"
                            thinking_placeholder.empty()
